...
void Tracking::SendPose(const Sophus::SE3f& pose) {
    Eigen::Vector3f translation = pose.translation();
    Eigen::Quaternionf rotation(pose.unit_quaternion());

    Json::Value root;
    root["pose"]["x"] = translation.x();
    root["pose"]["y"] = translation.y();
    root["pose"]["z"] = translation.z();
    root["pose"]["qw"] = rotation.w();
    root["pose"]["qx"] = rotation.x();
    root["pose"]["qy"] = rotation.y();
    root["pose"]["qz"] = rotation.z();

    // Convert to string
    Json::StreamWriterBuilder wbuilder;
    std::string pose_json = Json::writeString(wbuilder, root);

    wsClient->send(pose_json);
}
...
void Tracking::Track()
{
    ...
    if(mState==OK || mState==RECENTLY_LOST) {
        ...
        if(mCurrentFrame.isSet()) {
            ...
            // Send pose information over WebSocket
            SendPose(mCurrentFrame.GetPose());
        }
        else {
